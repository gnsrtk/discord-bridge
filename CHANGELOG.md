...
- コントロールパネルの Start ボタンでプロジェクトが起動しない不具合を修正 —
  `tmux new-window -t session` のセッション指定に末尾コロンが欠けており、
  tmux 3.6a が既存 window index への作成と解釈してエラーになっていた。
  `-t session:` に修正して次の空きインデックスに作成されるように
