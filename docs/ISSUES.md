...
| 43 | 🗑️ | ~~アシスタントがプレーンテキストで「〜しますか？」と質問した場合、Discord ではボタンにならず通常メッセージとして表示される~~ | `hooks/stop.py` | v1.3 で質問パターン自動検出を追加したが、言語依存のため #49 で削除。質問ボタン化は `AskUserQuestion`（pre_tool_use.py）に一本化 |
...
| 47 | ✅ | スレッドからのメッセージが親チャンネルの Claude Code セッションに送られ、スレッドごとの独立した会話ができない | `src/bot.ts`, `src/tmux-sender.ts`, `hooks/lib/thread.py` | スレッドごとに tmux ペインを自動作成（`tmux split-window`）し、`DISCORD_BRIDGE_THREAD_ID` 環境変数付きで独立した Claude Code セッションを起動。`threadPaneMap` でスレッド→ペインのマッピングを管理。`threadPaneCreating` Set でレース条件を防止。送信失敗時はペインを削除して次回再作成するセルフヒーリング。`ThreadUpdate` イベントでアーカイブ時にペインを自動終了 |
| 48 | ✅ | 複数行テキストを Discord から送ると `[Pasted text #N +M lines]` 表示後に Claude Code が入力待ちのまま止まる。bracketed paste シーケンス送信後の Enter が Claude Code のイベントループ処理完了前に届き無視される | `src/tmux-sender.ts` | `send-keys -l` で `\x1b[200~text\x1b[201~` を送信後、100ms 待機（`Atomics.wait`）してから `send-keys Enter` を送るよう変更。`paste-buffer` の libevent 非同期 I/O を回避し、同一 `send-keys` 経路で順序を保証 |
| 49 | ✅ | 日本語質問パターン自動検出が言語依存で他言語対応の障壁になっている | `hooks/stop.py` | 質問パターン自動検出機能（`QUESTION_PATTERN` / `BINARY_QUESTION_PATTERN` / `is_question()` / `post_message_with_buttons()`）を完全削除。質問のボタン化は `AskUserQuestion`（pre_tool_use.py）に一本化 |
| 50 | ✅ | Claude がツール実行前の思考テキストを Discord に送信しない。Stop hook は応答完了後にのみ発火するため、途中経過が見えない | `hooks/pre_tool_progress.py`（新規） | PreToolUse hook として `pre_tool_progress.py` を追加。ツール実行前に transcript から最新アシスタントテキストを取得し `🔄` プレフィックス付きで非同期送信。送信コンテンツの MD5 ハッシュで重複防止 |
